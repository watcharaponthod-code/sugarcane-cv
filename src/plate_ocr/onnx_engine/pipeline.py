...
class ThaiLPR:
    ...
    def __init__(self, engine=C.OCR_ENGINE, debug_dir=None,
                 detector="opencv", yolo_model=None):
        """detector: 'opencv'      (ไม่ต้องใช้โมเดล ML)
                     'onnx'        (pip install open-image-models — YOLOv9 สำเร็จรูป)
                     'onnx+opencv' (แนะนำ — ONNX ก่อน ไม่เจอค่อยเติมด้วย opencv)
                     'yolo'   (ต้อง pip install ultralytics + ไฟล์โมเดล .pt)"""
        self.reader = PlateReader(engine)
        self.debug_dir = debug_dir
        self.detector = detector
        self.yolo_model = yolo_model
        if detector == "yolo" and not yolo_model:
            raise ValueError("detector='yolo' ต้องระบุ yolo_model=path/to/best.pt")
        print(f"[ThaiLPR] OCR engine: {self.reader.engine} | "
              f"detector: {detector}")

    def _detect(self, bgr):
        if self.detector in ("onnx", "onnx+opencv"):
            from .detector_onnx import detect_plates_onnx
            cands = detect_plates_onnx(bgr, debug_dir=self.debug_dir)
            if self.detector == "onnx":
                return cands
            # cascade: โมเดล ONNX แม่นแต่บางภาพหาไม่เจอ -> เติมด้วย detector เดิม
            # (ตัวที่ซ้อนกับของ ONNX จะถูกตัดทิ้งใน NMS ท้าย pipeline อยู่แล้ว)
            seen = list(cands)
            for c in detect_plates(bgr, self.debug_dir):
                if all(self._box_overlap(c["bbox"], k["bbox"]) <= 0.5 for k in seen):
                    seen.append(c)
            return seen[:C.MAX_CANDIDATES]
        if self.detector == "yolo":
            from .detector_yolo import detect_plates_yolo
            return detect_plates_yolo(bgr, self.yolo_model,
